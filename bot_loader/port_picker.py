...
def pick_unused_ports(num_ports, retry_interval_secs=1, retry_attempts=5):
    ...
    for _ in range(retry_attempts):
        ports.update(portpicker.pick_unused_port() for _ in range(num_ports - len(ports)))
...
def pick_contiguous_unused_ports(num_ports, retry_interval_secs=1, retry_attempts=5):
